...
void DbConnection::create_tables() const {
    ...
    const std::string sql_cinema_room = "create table if not exists cinema_rooms (id integer primary key autoincrement, name varchar(50) not null, cinema_id integer, foreign key (team_id) references cinemas (id) on delete cascade on update cascade, row integer, places integer";
    ...
    const std::string sql_seat = "create table if not exists seats (id integer primary key autoincrement, cinema_room_id integer, foreign key (cinema_room_id) references cinema_rooms (id) on delete cascade on update cascade, row integer, place integer";
    ...
    const std::string sql_seance = "create table if not exists seances (id integer primary key autoincrement, movie_id integer, foreign key (movie_id) references movies (id) on delete cascade on update cascade, cinema_room_id integer, foreign key (cinema_room_id) references cinema_rooms (id) on delete cascade on update cascade, date_time datetime)";
    ...
    const std::string sql_ticket = "create table if not exists tickets (id integer primary key autoincrement, customer_id integer, foreign key (customer_id) references customers (id) on delete cascade on update cascade, seance_id integer, foreign key (seance_id) references seances (id) on delete cascade on update cascade, seat_id integer, foreign key (seat id) references seats (id) on delete cascade on update cascade, price integer, state varchar(50) not null)";
